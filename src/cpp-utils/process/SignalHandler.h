...
void wrap_signal_handler(int signal) {
    SignalHandlerRunningRAII disable_signal_processing_while_handler_running_and_reset_handler_afterwards;
    (*handler)(signal);
}
}
...
public:
    explicit SignalHandlerRAII(int signal)
    : _handler(handler), _old_handler(nullptr), _signal(signal) {
